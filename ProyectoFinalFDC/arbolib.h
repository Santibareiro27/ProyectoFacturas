...
Nodo *rightRotate(Nodo *y) {
	// Funcion para rotar a la derecha el  subárbol con raiz y
	Nodo *x = y->left;
	Nodo *T2 = x->right;
	// Realizar rotacion
	x->right = y;
...
void GUARDADO(Nodo *root, FILE *arch) {
	//func: recorre el arbol cargando datos de clientes al archivo de clientes
...
int GUARDAR_CLIENTES(Nodo *root, char *archname) {
	//func: guarda los clientes en la base de datos
